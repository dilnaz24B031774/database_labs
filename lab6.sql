...
--Answer to question: -- Query 1 (filter in ON) keeps all employees but shows NULL for non-Building A departments,
-- while Query 2 (filter in WHERE) excludes those employees entirely, acting like INNER JOIN.
--7.3
--with INNER JOIN
SELECT e.emp_name ,d.dept_name ,e.salary FROM employees e
...
--8.1
SELECT d.dept_name,e.emp_name,p.project_name,p.budget FROM departments d
LEFT JOIN employees e ON d.dept_id = e.dept_id
...
-- 1. INNER JOIN returns only matching rows; LEFT JOIN returns all rows from the left table plus matches.
-- 2. Use CROSS JOIN to create every possible combination, e.g. schedule employees vs. projects.
-- 3. In OUTER JOINs, ON filters before join keeping unmatched rows; WHERE filters after join removing them; in INNER JOIN both give same result.
-- 4. SELECT COUNT(*) FROM table1 CROSS JOIN table2 = 5 * 10 = 50 rows.
-- 5. NATURAL JOIN joins tables on all columns with the same names.
-- 6. Risk: unexpected joins if columns share names unintentionally, leading to wrong results.
-- 7. RIGHT JOIN version: SELECT * FROM B RIGHT JOIN A ON A.id = B.id;
-- 8. Use FULL OUTER JOIN when you need all rows from both tables, matched and unmatched.